...
% \subsection{Our Contributions}
    ...
    The dominant cost of YIN is the evaluation of $d(\tau)$ for every lag and every window, an $\mathcal{O}(W \cdot \tau_{\max})$ operation. We applied three targeted optimisations to reduce this cost in practice.
    ...
    \paragraph{Vectorisation.} Within each window, the inner summation $\sum_j (x[j] - x[j+\tau])^2$ is a straightforward fused subtract-square-accumulate pattern that maps directly to SIMD instructions.
...
%(vielleicht weglassen?):\\
    ...
    \paragraph{Predefined Twiddles} In our FFT implementation, computing the twiddle factors
