...
$$
d'(\tau) = \begin{cases} 1 & \tau = 0, \\ \displaystyle\frac{d(\tau) \cdot \tau}{\sum_{j=1}^{\tau} d(j)} & \tau > 0. \end{cases}
$$
...
\textbf{Parallelisation.} Each window is processed independently — the difference function, CMND computation, and minimum search for window $k$ do not depend on any other window.
...
\textbf{Vectorisation.} Within each window, the inner summation $\sum_j (x[j] - x[j+\tau])^2$ is a straightforward fused subtract-square-accumulate pattern that maps directly to SIMD instructions.
...
\textbf{Downsampling.} Pitch detection does not require the full audio bandwidth; only frequencies up to the maximum expected pitch are relevant.
...
To avoid aliasing, we apply a zero-phase FIR low-pass filter prior to downsampling: the signal is convolved with a 101-tap Hamming-windowed sinc filter in both the forward and backward directions, yielding a linear-phase response with no phase distortion.
