...
TEST(PythonBindingsTest, MatchMidi) {
    ...
    EXPECT_NO_THROW({
        p2t::matchMidi(wavFile, midiFile, outputFile);
    });
}

TEST(PythonBindingsTest, RoundToScale) {
    std::string wavFile = constants::VOICE_F400_SR4100;
    std::string outputFile = constants::TEST_OUTPUT_DIR + "/test_tune_to_scale.wav";
    p2t::Scale scale = p2t::Scale::fromName("C major");

    EXPECT_NO_THROW({
        p2t::roundToScale(wavFile, scale, outputFile);
    });
}
