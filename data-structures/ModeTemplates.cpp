...
// ---------------------- ScaleUtils implementation ----------------------
    float EqualTemperamentUtils::semitoneRatio(const int semitone, const float division, const float repeat) {
        return std::min(std::max(std::pow(repeat, static_cast<float>(semitone) / division), 1.0f), repeat);
    }

    std::vector<float> EqualTemperamentUtils::semitoneRatios(const std::vector<int>& semitones, float division, float repeat) {
        std::vector<float> ratios(semitones.size());
        std::transform(semitones.begin(), semitones.end(),
                       ratios.begin(),
                       [division, repeat](const int s){ return semitoneRatio(s, division, repeat); });
        return ratios;
    }
