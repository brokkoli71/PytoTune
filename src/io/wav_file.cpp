...
void WavFile::readFile(const std::string &path) {
    ...
    if (numSamples > 1) {
        std::vector<float> monoSamples(numSamples / numChannels);
        for (size_t i = 0; i < monoSamples.size(); ++i) {
            monoSamples[i] = samples[i * numChannels];
        }
    ...
    wavData.samples = std::move(samples);
}
...
void WavFile::store(const std::string &path, uint16_t audioFormat) const {
    ...
    // Number of Channels
    writeData(&wavData.numChannels, sizeof(wavData.numChannels));

    // Sample Rate
    writeData(&wavData.sampleRate, sizeof(wavData.sampleRate));

    // Byte Rate (Sample Rate * Num Channels * Bits Per Sample / 8)
    uint32_t byteRate = wavData.sampleRate * wavData.numChannels * (audioFormat == 1 ? 2 : 4);
    // 2 bytes for 16-bit, 4 bytes for float
    writeData(&byteRate, sizeof(byteRate));

    // Block Align (Num Channels * Bits Per Sample / 8)
    uint16_t blockAlign = wavData.numChannels * (audioFormat == 1 ? 2 : 4);
    // 2 bytes for 16-bit, 4 bytes for float
    ...
    uint32_t dataChunkSize = wavData.samples.size() * (audioFormat == 1 ? 2 : 4) * wavData.numChannels;
    writeData(&dataChunkSize, sizeof(dataChunkSize));

    // Writing audio samples
    if (audioFormat == 1) {
        // PCM 16-bit
        std::vector<int16_t> pcmData(wavData.samples.size());
        for (size_t i = 0; i < wavData.samples.size(); ++i) {
            pcmData[i] = static_cast<int16_t>(wavData.samples[i] * 32768.0f);  // Scale to 16-bit PCM range
        }
        writeData(pcmData.data(), pcmData.size() * sizeof(int16_t));
    } else if (audioFormat == 3) {
        // Float 32-bit
        writeData(wavData.samples.data(), wavData.samples.size() * sizeof(float));
    }
