...
//
#include "../../include/pytotune/algorithms/PitchShifter.h"
...
namespace p2t {
    std::vector<float> PitchShifter::run(const std::vector<float> samples, float factor) const {
        const int buffer_size = static_cast<int>(std::pow(2, std::ceil(std::log2(factor)))) * 2 * windowing.windowSize;
