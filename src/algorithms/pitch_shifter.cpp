...
std::vector<float> PitchShifter::run(const std::vector<float> &samples,
    ...
    const int buffer_size = static_cast<int>(std::pow(2, std::ceil(std::log2(maxPitchFactor)))) * 2 * windowing.windowSize;
    ...
    float freqPerBin = windowing.sampleRate / static_cast<float>(windowing.windowSize);
