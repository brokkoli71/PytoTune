...
Scale Scale::fromName(const std::string &name, float tuning) {
    ...
    static const std::unordered_map<std::string, int> NOTE_SEMITONES = {
        {"Cb", 2}, {"C", 3}, {"C#", 4}, {"Db", 4}, {"D", 5}, {"D#", 6}, {"Eb", 6}, {"E", 7}, {"E#", 8}, {"Fb", 7}, {"F", 8}, {"F#", 9}, {"Gb", 9}, {"G", 10}, {"G#", 11}, {"Ab", 11}, {"A", 0}, {"A#", 1}, {"Bb", 1}, {"B", 2}, {"B#", 3}};

    if (NOTE_SEMITONES.count(note) == 0)
        throw std::invalid_argument("Invalid root note. It mus have the format: [A|B|C|D|E|F|G](#|b)?");

    return Scale::fromModeName(
        modeName, tuning * std::pow(2.0f, static_cast<float>(NOTE_SEMITONES.at(note)) / 12.0f));
}
